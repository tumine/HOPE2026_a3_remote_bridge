    ...
    for trial in range(args.num_serves):
        ...
        if args.training_aligned_preroll:
            ...
            for _ in range(prelaunch_ticks):
                ...
                _park_ball()
            # With zero hold and a flight horizon exactly equal to the policy
            # lead there is no prelaunch policy tick to consume the new task.
            # Engage it without advancing physics or the strike clock so the
            # first post-launch actor observation still sees the full TTS.
            if lifecycle.active_task_id != task_id:
                lifecycle.update(source.poll(), scene.read_robot_state())
