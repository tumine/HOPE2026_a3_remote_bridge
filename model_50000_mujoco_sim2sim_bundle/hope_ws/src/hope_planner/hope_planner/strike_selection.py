...
class StrikeRegion:
    ...
    def contains(self, position: np.ndarray, *, atol: float = 1.0e-9) -> bool:
        p = np.asarray(position, dtype=np.float64)
        if p.shape != (3,) or not np.all(np.isfinite(p)):
            return False
        return (
            self.x[0] - atol <= p[0] <= self.x[1] + atol
            and self.y[0] - atol <= p[1] <= self.y[1] + atol
            and self.z[0] - atol <= p[2] <= self.z[1] + atol
        )

    def contains_velocity(self, velocity: np.ndarray, *, atol: float = 1.0e-9) -> bool:
        ...
        if v.shape != (3,) or not np.all(np.isfinite(v)):
            return False
        return bool(
            np.all(v >= limits[:, 0] - atol) and np.all(v <= limits[:, 1] + atol)
        )
    ...
    def contains_command(
        ...
        *,
        ...
        return self.contains(position, atol=atol) and self.contains_velocity(
            velocity, atol=atol
        )
    ...
    def rejection_reason(
        ...
        if velocity is not None:
            ...
            if v.shape != (3,) or not np.all(np.isfinite(v)):
                ...
                for index, (axis, limits) in enumerate(zip("xyz", self.velocity)):
...
def validate_side_regions(
    ...
    if fh.y[1] > split_y - hysteresis_y:
        raise ValueError(
            "forehand y range extends above split-hysteresis: "
            f"max={fh.y[1]}, split-h={split_y - hysteresis_y}"
        )
    if bh.y[0] < split_y + hysteresis_y:
        raise ValueError(
            "backhand y range extends below split+hysteresis: "
...
def select_strike_candidate(
    ...
    *,
    ...
    for side, strike in candidates.items():
        ...
        if region is None or not strike.valid or not region.contains(strike.p_ball):
            continue
