...
class ExampleCommandFeed(RacketCommandSource):
    ...
    the swing. The targets are table/world hit points inside the moving-station
    coverage. The runtime inverses them into clip-centred racket reach plus a
    lateral base target; a real deployment replaces this entirely with the
    planner's ``RacketCommand`` stream. Target x/y are specified relative to the
    reset station and converted to MuJoCo world here; z is absolute floor-world.
    """
    ...
    # Both use the clip reach centre after station inversion, while requiring a
    # real lateral transition (-0.11 m forehand, +0.30 m backhand).
    forehand_pos: tuple = (0.290, -0.750, 1.025)
    backhand_pos: tuple = (0.600, 0.200, 1.025)
