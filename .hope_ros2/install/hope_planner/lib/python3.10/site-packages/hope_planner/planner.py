...
class HOPEPlanner:
    ...
    def __init__(
        ...
        self._latest_ball_position: Optional[np.ndarray] = None
        self._latest_ball_velocity: Optional[np.ndarray] = None
    ...
    def update_candidates(
        ...
        if not self.estimator.ready:
            ...
            self._latest_ball_velocity = None
            return {}

        p_est, v_est, t_est = self.estimator.estimate()
        self._latest_t = t_est
        self._latest_ball_position = np.asarray(p_est, dtype=np.float64).copy()
        self._latest_ball_velocity = np.asarray(v_est, dtype=np.float64).copy()
    ...
    @property
    def estimated_ball_position(self) -> Optional[np.ndarray]:
        """Latest fitted ball position, or ``None`` before the estimator is ready."""
        if self._latest_ball_position is None:
            return None
        return self._latest_ball_position.copy()

    @property
    def estimated_ball_velocity(self) -> Optional[np.ndarray]:
        """Latest fitted ball velocity, or ``None`` before the estimator is ready."""
        if self._latest_ball_velocity is None:
            return None
        return self._latest_ball_velocity.copy()
