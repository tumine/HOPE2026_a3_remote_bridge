...
- `pc_tools/a3_rl_deploy.py`：电脑端交互式 `OBSERVE -> DEFAULT_RAMP ->
  ...
  策略；真实来球由 `/ppmocap/frame -> hope_planner -> /racket/command` 自动触发，
...
- `scripts/run_a3_rviz.sh`：以 PPMocap 的球桌/球/轨迹场景为底稿，将动捕 BotA3
  ...
  0.76 m 的地面。
- `pc_tools/a3_rviz_pose_bridge.py`：复用策略的球桌标定和 marker-to-pelvis 外参，
  发布 `hope_table -> a3/pelvis_link` 动态 TF。
