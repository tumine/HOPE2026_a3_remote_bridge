...
训练中 10% 环境使用独立抬高手臂的中立 READY 参考动作，但部署观测仍使用上述固定正手条件锚点，两者不是同一个概念。`model_64500` 的 MuJoCo 无球测试虽然稳定，却没有准确跟踪该中立参考：腰部仍存在 yaw/roll 偏置、腰 pitch 输出明显前倾，脚底也没有保持水平。完整数值见 `VALIDATION.md`，在解决该差异前不要把本包直接视为真机安全认证策略。
