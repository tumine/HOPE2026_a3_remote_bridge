...
Two evaluation modes (``--eval-mode``):
  ...
  * ``continuous`` (default) — deploy-faithful continuous rally: the robot, policy
    ``last_action``, lifecycle and moving base target are initialized ONCE; between serves
...
# ---------------------------------------------------------------------------------------------------
# Example incoming serve distribution. Its interception points use the exact station-relative
# forehand/backhand target boxes from Isaac Lab training; every observed trajectory is passed through
# the shipped planner to produce the RacketCommand.
...
class _EvaluationDiagnostics:
    ...
    clipped_action_ticks: int = 0
    saturated_action_ticks: int = 0
    tanh_saturated_action_ticks: int = 0
    policy_ticks: int = 0
    max_abs_raw_action: float = 0.0
    max_abs_adapter_action: float = 0.0
    ...
    def as_dict(self) -> dict:
        ...
        return {
            ...
            "clipped_action_ticks": self.clipped_action_ticks,
            ...
            "tanh_saturation_fraction": self.tanh_saturated_action_ticks
            / max(self.policy_ticks, 1),
            "max_abs_raw_action": self.max_abs_raw_action,
            "max_abs_adapter_action": self.max_abs_adapter_action,
            "min_pelvis_height_m": (
...
def run_eval(args) -> dict:
    ...
    from a3_deploy_onnx_ref_pingpong.station_target import (
    ...
    if args.fixed_station:
        # The 2026-07-25 run predates moving-base station semantics. Keep this
        # opt-in so a shape-compatible legacy actor cannot silently be evaluated
        # with the newer moving-base observation meaning.
        runtime_cfg = replace(
            runtime_cfg,
            lateral_station=replace(runtime_cfg.lateral_station, enabled=False),
        )
    ...
    for flag, value, maximum in (
        ("--start-base-x-jitter", args.start_base_x_jitter, 0.10),
        ("--start-base-y-jitter", args.start_base_y_jitter, 0.35),
    ):
        if not np.isfinite(value) or value < 0.0 or value > maximum:
            raise ValueError(f"{flag} must be finite and lie in [0, {maximum}]")
    ...
    if runtime_cfg.lateral_station.enabled:
        validate_lateral_station_policy_manifest(onnx_path)
    ...
    scene = PingPongRealPhysicsScene(
        ...
        joint_velocity_limits=np.asarray(
            ISAAC_JOINT_VELOCITY_LIMITS, dtype=np.float64
        ),
    ...
    def _policy_tick(
        ...
        *,
        ...
        """
        nonlocal base_target_xy
        state = scene.read_robot_state()
        target = lifecycle.update(source.poll(), state)
        station_cfg = runtime_cfg.lateral_station
        if lifecycle.phase in (Phase.READY, Phase.RECOVERY):
            base_target_xy = np.asarray(fixed_station_xy, dtype=np.float64).copy()
        elif station_cfg.enabled and lifecycle.phase in (
            Phase.SWING,
            Phase.FOLLOW_THROUGH,
        ):
            base_target_xy = derive_lateral_base_target(
                target.pos_w,
                target.swing_side,
                fixed_station_xy,
        ...
        diagnostics.max_station_drift_m = max(
            ...
            float(np.linalg.norm(np.asarray(state.base_pos_w[:2]) - base_target_xy)),
        )
        ...
        # Applied action = shared raw-action clip followed by passive-head zeroing,
        # matching both the deploy runner and training's last_action feedback.
        applied_action = runtime_cfg.action_adapter.clip_raw_action(raw_action)
        if not np.array_equal(applied_action, np.asarray(raw_action, dtype=np.float64)):
            diagnostics.clipped_action_ticks += 1
        if runtime_cfg.passive_neck:
            applied_action[head_idx] = 0.0
        adapter_action = runtime_cfg.action_adapter.transform_raw_action(applied_action)
        diagnostics.max_abs_adapter_action = max(
            diagnostics.max_abs_adapter_action,
            float(np.max(np.abs(adapter_action))),
        )
        if (
            runtime_cfg.action_adapter.raw_action_transform == "tanh"
            and np.any(np.abs(adapter_action) >= 0.995)
        ):
            diagnostics.tanh_saturated_action_ticks += 1
        unclamped = (
            runtime_cfg.action_adapter.default_q
            + adapter_action * runtime_cfg.action_adapter.action_scale
        )
        if np.any(
    ...
    def _reset_robot_start():
        """Reset to stand, then sample a bounded offset from the nominal station."""

        scene.reset_stand()
        offset = np.array(
            [
                rng.uniform(-args.start_base_x_jitter, args.start_base_x_jitter),
                rng.uniform(-args.start_base_y_jitter, args.start_base_y_jitter),
            ],
            dtype=np.float64,
        )
        scene.offset_base_xy(offset)
        print(
            "[mujoco_eval] randomized start offset: "
            f"x={offset[0]:+.3f} m, y={offset[1]:+.3f} m",
            file=sys.stderr,
        )
        return offset

    # Continuous mode: ONE initialization for the whole session — robot state,
    # last_action, lifecycle and the moving base target all persist across serves
    # (matching the deploy runner). Independent mode re-creates them per serve.
    current_start_offset = _reset_robot_start()
    ...
    # The table and station remain anchored to the MJCF stand keyframe.  A
    # randomized robot start must appear to the actor as station error, not move
    # the virtual table centre with the pelvis.
    fixed_station_xy = np.asarray(stand_base_xy, dtype=np.float64).copy()
    base_target_xy = fixed_station_xy.copy()
    ...
    for trial in range(args.num_serves):
        # Coverage mode exercises all four adjacent transitions. Alternating mode
        # is a lateral stress test: every ball switches between forehand/backhand.
        if args.serve_side_pattern == "alternating":
        ...
        diagnostics.trials.append(
            {
                ...
                "initial_base_offset_xy": current_start_offset.tolist(),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fixed-station",
        action="store_true",
        help="Evaluate a legacy actor trained with a stationary base target. "
        "Use only with an actor whose manifest lacks moving_lateral_target_v1.",
    )
    parser.add_argument("--num-serves", type=int, default=50, help="Number of served balls (denominator).")
    parser.add_argument(
        "--serve-side-pattern",
        choices=["coverage", "alternating"],
        default="coverage",
        help="coverage (default): FH,FH,BH,BH to cover all transitions; "
        "alternating: FH,BH,FH,BH for maximum consecutive lateral switching.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--forehand-strike-height-min", type=float, default=0.84,
    ...
    parser.add_argument(
        "--backhand-strike-height-max", type=float, default=1.21,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Symmetric random floating-base X offset applied after stand reset (m, max 0.10).",
    )
    parser.add_argument(
        "--start-base-y-jitter",
        type=float,
        default=0.0,
        help="Symmetric random floating-base Y offset applied after stand reset (m, max 0.35).",
    )
