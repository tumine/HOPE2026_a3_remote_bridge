...
class PingPongRealPhysicsScene:
    ...
    def __init__(
        ...
        self._table_restitution = float(table_contact.get("restitution", 0.9448))
        self._table_tangential_damping = float(
            table_contact.get("tangential_damping", 0.2079)
        )
    ...
    def _build_model(self, mujoco, robot_xml_path, ball_cfg) -> None:
        ...
        _add_pair("ball_geom", "table_geom", _restitution("table", 0.9448), _friction("table", 0.40))
    ...
    def offset_base_xy(self, offset_xy) -> None:
        """Translate a freshly reset floating base without changing its nominal station.

        This is intended for evaluator start-state randomization.  The caller
        remains responsible for keeping the offset inside its task domain.
        """

        offset = np.asarray(offset_xy, dtype=np.float64).reshape(-1)
        if offset.shape != (2,) or not np.all(np.isfinite(offset)):
            raise ValueError("base XY offset must contain two finite values")
        self.data.qpos[self._base_qadr:self._base_qadr + 2] += offset
