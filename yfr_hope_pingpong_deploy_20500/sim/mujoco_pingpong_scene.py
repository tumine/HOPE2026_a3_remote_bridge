...
class PingPongRealPhysicsScene:
    ...
    def _build_model(self, mujoco, robot_xml_path, ball_cfg) -> None:
        # Current MuJoCo exposes a class-level constructor; some older bindings
        # expose an instance method. Preserve the returned spec in either case.
        try:
            spec = mujoco.MjSpec.from_file(str(robot_xml_path))
        except TypeError:
            spec = mujoco.MjSpec()
            loaded = spec.from_file(str(robot_xml_path))
        ...
        # mujoco>=3.2: add_freejoint takes no name kwarg (name derives from the body);
        # older releases accepted name=. Keep the joint name stable via the body name.
        fj = None
        try:
            fj = bb.add_freejoint(name="ball_free_joint")
        except TypeError:
            fj = bb.add_freejoint()
            # mujoco>=3.2: freejoint name derives from the body name as empty string;
            # restore the historical explicit name the rest of the scene relies on.
            try:
                fj.name = "ball_free_joint"
            except Exception:
                pass
        bb.add_geom(
        ...
        def _add_pair(g1: str, g2: str, e: float, fr: float) -> None:
            # mujoco>=3.2: add_pair() takes no geom kwargs; assign names via attributes.
            pair = spec.add_pair()
            pair.geomname1 = g1
            pair.geomname2 = g2
        ...
        # MuJoCo 3.11 exposes ``spec.keys`` as the list and ``spec.key`` as a
        # name-lookup method. Older bindings used a list-like ``spec.key``.
        keyframes = getattr(spec, "keys", None)
        if keyframes is None:
            candidate = getattr(spec, "key", None)
            keyframes = None if callable(candidate) else candidate
        if keyframes:
            kf = keyframes[0]
            kf.qpos = list(np.array(kf.qpos, dtype=np.float64)) + [
