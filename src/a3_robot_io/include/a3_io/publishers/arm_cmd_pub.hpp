...
// 14-DOF arm command publisher.
// Source slice: RobotCommand.[...][5..18] (kA3ArmStart=5) in MuJoCo real
// 31-DOF SDK layout.
class ArmCmdPub {
 public:
  static constexpr std::size_t kDof = robot_io::kA3ArmCount;  // 14

#ifdef HAS_A3_ROS_MSGS
  using PublishFn = std::function<void(const joint_msgs::msg::JointCommand&)>;

  ArmCmdPub() = default;
  explicit ArmCmdPub(PublishFn publish) : publish_fn_(std::move(publish)) {}
