...
# Isaac ``velocity_limit_sim`` in the same canonical order.  The legacy aligned
# MJCF embeds this tuple as a custom numeric; externally supplied, higher-fidelity
# robot models may omit that evaluator-specific metadata, so the reference bridge
# can pass this contract explicitly without modifying the source model.
ISAAC_JOINT_VELOCITY_LIMITS: tuple[float, ...] = (
    12.0, 22.7, 9.2,
    12.7, 12.7,
...
assert len(ISAAC_JOINT_VELOCITY_LIMITS) == NUM_JOINTS
