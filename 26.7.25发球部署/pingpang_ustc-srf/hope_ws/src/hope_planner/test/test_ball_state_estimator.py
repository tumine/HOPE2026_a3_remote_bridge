...
def test_explicit_reset_clears_samples_and_bounce_history():
    cfg = PlannerConfig()
    est = BallStateEstimator(cfg)
    est.push(0.0, np.array([1.0, 0.0, 0.10]))
    est.push(0.01, np.array([0.9, 0.0, 0.02]))
    est.reset()
    est.push(1.0, np.array([0.8, 0.0, 0.10]))
    assert not est.bounce_detected
    assert len(est.t_buffer) == 1
    assert not est.ready
