...
def test_per_call_plane_override_does_not_mutate_config():
    pred = _predictor()
    configured_x = pred.config.x_hit
    p0 = np.array([0.5, -0.7625, 0.5])
    v0 = np.array([-3.0, 0.0, 2.0])

    near = pred.predict(p0, v0, 0.0, x_hit=0.2)
    far = pred.predict(p0, v0, 0.0, x_hit=-0.2)

    assert near.valid and far.valid
    assert near.p_ball[0] == pytest.approx(0.2)
    assert far.p_ball[0] == pytest.approx(-0.2)
    assert near.t_strike < far.t_strike
    assert pred.config.x_hit == configured_x
