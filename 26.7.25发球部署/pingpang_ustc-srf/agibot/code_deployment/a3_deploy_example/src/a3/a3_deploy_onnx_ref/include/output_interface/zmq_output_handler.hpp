 ...
 *
 * The topic prefix is a plain string (e.g. "a3_debug") prepended to the
 * message so that subscribers using `zmq::sockopt::subscribe` can filter by
 * topic.
 *
 * ---------------------------------------------------------------------------
 * ## `{user_topic}` (e.g. `a3_debug`) — published every tick
 * ---------------------------------------------------------------------------
 *
...
public:
    ...
    ) override
    {
        ...
        // 3. Send once on user topic (e.g. "a3_debug")
     ...
     *
     * Called once during init (from a3_deploy_onnx_ref.cpp) and then on every
...
private:
    ...
    std::string topic_;              ///< User-provided topic name (e.g. "a3_debug") for combined state+viz.
