 ...
 *   - **Output**: single tensor `action` (float32, dimension = A3_POLICY_DOF = 29).
 *
 * ## Typical Usage
 *
...
public:
  ...
  bool Initialize(const std::string& model_path, bool use_fp16 = false) {
    ...
    try {
      ...
      if (config_.action_dimension != A3_POLICY_DOF) {
        std::cerr << "✗ Policy action dimension (" << config_.action_dimension 
                  << ") doesn't match A3 robot motors (" << A3_POLICY_DOF << ")" << std::endl;
        inference_engine_.reset();
        return false;
      }
