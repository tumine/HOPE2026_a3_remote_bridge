...
public:
    ...
    bool ResampleGeneratedSequence50Hz()
    {
        ...
        if(timesteps_30hz > 64)
        {
            ...
            /*
            for(int i = 0; i < (A3_POLICY_DOF + 7) * timesteps_30hz; i++)
            {
            ...
            cnpy::npy_save("context_qpos.npy", GetContextBuffer(), std::vector<size_t>{4, (A3_POLICY_DOF + 7)}, "w");
            */
        ...
        if(
            std::any_of(
                mujoco_qpos_data, mujoco_qpos_data + (A3_POLICY_DOF + 7) * timesteps_30hz,
        ...
        for(int32_t f_50hz = 0; f_50hz < planner_motion_50hz_.timesteps; ++f_50hz)
        {
            ...
            planner_motion_50hz_.BodyPositions(f_50hz)[0] = std::array<double, 3> {
                w0 * mujoco_qpos_data[f0 * (A3_POLICY_DOF + 7) + 0] + w1 * mujoco_qpos_data[f1 * (A3_POLICY_DOF + 7) + 0],
                w0 * mujoco_qpos_data[f0 * (A3_POLICY_DOF + 7) + 1] + w1 * mujoco_qpos_data[f1 * (A3_POLICY_DOF + 7) + 1],
                w0 * mujoco_qpos_data[f0 * (A3_POLICY_DOF + 7) + 2] + w1 * mujoco_qpos_data[f1 * (A3_POLICY_DOF + 7) + 2]
            };
            
            // resample global body quaternion:
            std::array<double, 4> q0 = {
                mujoco_qpos_data[f0 * (A3_POLICY_DOF + 7) + 3],
                mujoco_qpos_data[f0 * (A3_POLICY_DOF + 7) + 4],
                mujoco_qpos_data[f0 * (A3_POLICY_DOF + 7) + 5],
                mujoco_qpos_data[f0 * (A3_POLICY_DOF + 7) + 6]
            };
            std::array<double, 4> q1 = {
                mujoco_qpos_data[f1 * (A3_POLICY_DOF + 7) + 3],
                mujoco_qpos_data[f1 * (A3_POLICY_DOF + 7) + 4],
                mujoco_qpos_data[f1 * (A3_POLICY_DOF + 7) + 5],
                mujoco_qpos_data[f1 * (A3_POLICY_DOF + 7) + 6]
            };
            ...
            for(int joint = 0; joint < 29; ++joint) {
                planner_motion_50hz_.JointPositions(f_50hz)[joint] =
                    w0 * mujoco_qpos_data[f0 * (A3_POLICY_DOF + 7) + 7 + mujoco_to_isaaclab[joint]] + 
                    w1 * mujoco_qpos_data[f1 * (A3_POLICY_DOF + 7) + 7 + mujoco_to_isaaclab[joint]];
            }
        }
    ...
    )
    {
        ...
        for(int n = 0; n < 4; ++n) {
            int index_base = n * (A3_POLICY_DOF + 7);
    ...
    void UpdateContextFromMotion(const std::shared_ptr<const MotionSequence>& motion_sequence) {
        ...
        for(int n=0; n<4; ++n)
        {
            ...
            std::copy(quat.begin(), quat.end(), &context_qpos_values[n * (A3_POLICY_DOF + 7) + 3]);
            ...
            context_qpos_values[n * (A3_POLICY_DOF + 7) + 0] = p[0];
            context_qpos_values[n * (A3_POLICY_DOF + 7) + 1] = p[1];
            context_qpos_values[n * (A3_POLICY_DOF + 7) + 2] = p[2];

            // resample joint positions
            for(int i = 0; i < A3_POLICY_DOF; i++) {
                context_qpos_values[n * (A3_POLICY_DOF + 7) + 7 + mujoco_to_isaaclab[i]] = w0 * motion_sequence->JointPositions(f0)[i] + w1 * motion_sequence->JointPositions(f1)[i];
            }
        }
    }
