 ...
 *   -----------------------------|------------------|----------------------------
 *   A3Env/env_state_act          | depth=10         | Per-tick robot state + action data (msgpack).
 *   WBCPolicy/robot_config       | transient_local  | One-shot robot configuration (msgpack).
 *
 * ## State Topic (`A3Env/env_state_act`)
 *
...
#include "../policy_parameters.hpp"  // For isaaclab_to_mujoco, default_angles, legacy_action_scale
...
public:
     ...
     * @param node_name  ROS 2 node name (default: "a3_output_handler").
     * @throws std::exception if ROS 2 initialisation or node creation fails.
     */
    explicit ROS2OutputHandler(StateLogger& logger, const std::string& node_name = "a3_output_handler") 
    ...
    /// Publish the latest robot state to `A3Env/env_state_act` (called each control tick).
    void publish(
...
private:
    ...
    /// Publisher for per-tick robot state (A3Env/env_state_act).
    ...
    void setup_publishers() {
        ...
        if constexpr (DEBUG_LOGGING) {
            std::cout << "[ROS2 Output DEBUG] Created publisher for A3Env/env_state_act" << std::endl;
        }
    ...
    /// Publish the latest state-logger entry to `A3Env/env_state_act`.
    ...
    std::vector<uint8_t> parse_state_logger_state_to_msgpack(const DataBuffer<HeadingState>& heading_state_buffer) {
        ...
        if (state.last_action.size() == 29) {
            ...
            for (size_t i = 0; i < 29; ++i) {
                ...
                last_action_mujoco[i] = state.last_action[isaaclab_to_mujoco[i]] * legacy_action_scale[i] + default_angles[i];
            }
            // Pack in MuJoCo order
