...
| `[65:96]`   | `last_action`            | 31 | raw | Previous clipped **applied** action; passive head columns (idx 3, 4) are zero. |
...
| `[109:110]` | `time_to_strike`         | 1  | s | Time remaining at this pre-transition observation until the strike. Live command transport/queue age has already been subtracted. |
...
Each tick the actor emits `raw_action[31]` in the joint order below. It is clipped to
`[-100, 100]`, then the two passive head columns (idx 3, 4) are zeroed to form the
**applied action**, which is:
...
The public adapter is a linear joint-position residual with one scale shared by all 31 columns:
...
The action clip, transform, and constants (`default_q`, uniform `action_scale`, clamp limits) live in a single shared
config, `a3_deploy/a3_deploy_example/config/action_adapter.yaml`, read by **both**
training and the reference runner so you edit them in one place. `identity` adds no nonlinear
compression; `last_action` is the clipped action with the passive head columns zeroed. The
mechanical clamp is the final target bound and never emits a failure/rejection status.

This mapping is part of the trained policy contract. Changing its transform, default pose,
or scale requires retraining. In particular, checkpoints trained with the former
`tanh`/per-joint mapping are incompatible. The frozen 29k deployment bundle remains an
independent reproducibility snapshot.
...
When no `RacketCommand` is available, the current reference runtime does not
invent a zero-velocity reach. On the first simulator frame it converts the
configured `ready_target_rel_base_w` into one fixed world target. Later READY
observations recompute that fixed target minus the live base position, matching
training's station-sampled world target rather than moving the virtual target
with the robot. `ready_target_vel_w`, `ready_time_to_strike`, and
`ready_swing_side` remain fixed. Proprioception, `last_action`, and fixed-station
error remain live. A new command immediately replaces this READY target for the
active swing.
