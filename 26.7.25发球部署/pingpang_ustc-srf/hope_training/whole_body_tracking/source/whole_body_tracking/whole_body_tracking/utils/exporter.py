...
debug heads, and no multi-layout contract table. The deployment API always feeds raw observations.
When training used empirical observation normalization, its frozen normalizer is folded into the
ONNX graph; the external ``observation_normalization`` contract therefore remains ``none`` and the
optional ``embedded_observation_normalization`` field records that preprocessing. See
``policy_manifest.json`` and ``docs/POLICY_INTERFACE.md`` for the authoritative contract.
"""
...
ACTION_DIM = 31
CONTROL_RATE_HZ = 50
# The ActionAdapter contract (action clip/transform, default_q, scale, joint clamp limits)
# are referenced by path and read by BOTH training and the reference runner. They are not baked
# into the policy.
...
def build_manifest(
    ...
    embedded_observation_normalization: str = "none",
    provenance: dict | None = None,
) -> dict:
    """Assemble the ``policy_manifest.json`` dictionary for the exported policy."""
    if observation_normalization != "none":
        raise ValueError(
            "the deploy contract accepts raw observations only; fold training normalization into "
            "the ONNX and keep observation_normalization='none'"
        )
    if embedded_observation_normalization not in {"none", "empirical"}:
        raise ValueError(
            "embedded_observation_normalization must be either 'none' or 'empirical'"
        )
    ...
    if embedded_observation_normalization != "none":
        manifest["embedded_observation_normalization"] = embedded_observation_normalization
...
def write_policy_manifest(
    ...
    manifest = build_manifest(
        ...
        onnx_filename,
        embedded_observation_normalization,
        provenance,
    )
...
def attach_onnx_metadata(
    ...
    if embedded_observation_normalization != "none":
        meta["embedded_observation_normalization"] = embedded_observation_normalization
...
def export_policy(
    ...
    Returns ``(onnx_path, manifest_path)``. Pass the frozen actor normalizer when the checkpoint
    contains one. It is folded into the graph, so deployment still sends raw observations.
    """
    import torch.nn as nn

    embedded_norm = "none" if (normalizer is None or isinstance(normalizer, nn.Identity)) else "empirical"
    onnx_path = export_policy_as_onnx(actor_critic, path, onnx_filename, normalizer, verbose)
    # This field describes preprocessing expected FROM the deployment caller. Empirical
    # normalization, when present, already lives inside the ONNX and must not be applied twice.
    attach_onnx_metadata(
        onnx_path,
        joint_names,
        "none",
        embedded_observation_normalization=embedded_norm,
    )
