...
(ball<->table / net / floor / racket); optional no-spin quadratic drag is refreshed once per control
step by :class:`.table_tennis_env.TableTennisEnv` using :mod:`.ball`.
...
class TableTennisEnvCfg(ManagerBasedRLEnvCfg):
    ...
    # No-spin ball drag is refreshed once per control step and held across its decimated physics
    # substeps. It is opt-in because Isaac Sim 4.5 can fail in the GPU external-force API on
    # multi-card workstations. Gravity + contacts remain fully simulated when disabled.
    ball_aerodynamics: BallAerodynamicsCfg = BallAerodynamicsCfg.from_physics_config(enabled=False)
    ...
    def __post_init__(self):
        ...
        # the drag force is held constant over the four substeps. CCD is a scene-level PhysX flag.
