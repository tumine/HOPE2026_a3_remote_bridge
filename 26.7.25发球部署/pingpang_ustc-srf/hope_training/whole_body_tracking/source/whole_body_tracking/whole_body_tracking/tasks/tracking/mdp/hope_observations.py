...
def applied_last_action(env: ManagerBasedRLEnv, action_name: str = "joint_pos") -> torch.Tensor:
    """Previous tick's clipped/applied action (31), including zeroed passive-head columns.

    Reads the action term's deploy-faithful ``applied_raw_actions`` rather than Isaac Lab's global
    ``last_action`` so raw-action clipping and passive-head zeroing are reflected in feedback.
    """
...
def racket_pos_b(env: ManagerBasedRLEnv, command_name: str) -> torch.Tensor:
    ...
    cmd.refresh_racket_state()
    return quat_apply_inverse(yaw_quat(cmd.base_quat_w), cmd.racket_pos_w - cmd.base_pos_w)
