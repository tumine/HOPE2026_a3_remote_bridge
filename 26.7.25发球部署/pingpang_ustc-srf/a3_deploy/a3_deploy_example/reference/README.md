...
`--duration N` / `--max-ticks N`, `--idle` (no command feed; the policy receives
the configured training-distribution READY anchor).
...
| `action_adapter.py` | Shared ActionAdapter: `adapter_action = clip(raw, -100, 100)`; `q_des = default_q + adapter_action*0.25`, then clamp. |
...
5. clip to `[-100, 100]`, zero the passive head columns (idx 3, 4) to form the
   **applied action**, and feed that back as the next `last_action` (matching training);
...
In READY, the shipped current-policy runtime uses the explicit
`ready_target_rel_base_w` from `config/hope_pingpong_runtime.yaml` to lock one
world target on the simulator's first frame. It then subtracts the live base each
tick, so base drift changes the relative-position observation exactly as it did
for the fixed-station target in training. `ready_target_vel_w`,
`ready_time_to_strike`, and `ready_swing_side` remain fixed. These form a
policy-conditioning anchor sampled from the training command distribution; the
non-zero target velocity is an input condition, not measured idle racket motion.
Older runtime YAMLs without the new fields retain the original
`ready_reach_x/y/z` plus zero-velocity fallback.
