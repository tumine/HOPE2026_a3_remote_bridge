...
class SwingLifecycle:
    def __init__(self, cfg: LifecycleConfig | None = None) -> None:
        ...
        self._follow_t = 0.0
        self._recover_t = 0.0
        self._ready_target_pos_w_fixed: np.ndarray | None = None
    ...
    # -- helpers ------------------------------------------------------------
    def lock_ready_reference(self, base_pos_w: np.ndarray) -> None:
        """Freeze the simulator first-frame READY target in world coordinates."""

        base = np.asarray(base_pos_w, dtype=np.float64)
        ...
        if self.cfg.ready_target_rel_base_w is not None:
            self._ready_target_pos_w_fixed = base + np.asarray(
    ...
    def _ready_target_pos_w(self, state: RobotState) -> np.ndarray:
        ...
        if self.cfg.ready_target_rel_base_w is not None:
            if self._ready_target_pos_w_fixed is None:
                self.lock_ready_reference(base)
            assert self._ready_target_pos_w_fixed is not None
            return self._ready_target_pos_w_fixed.copy()
